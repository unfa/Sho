...
func _process(delta):
	...
	if close and not close_prev:
		anim.play("Close")
				
	if not close and not reject:
		
		if not open:
			
			if near and not collect:
				collect_all_stars()
...
#		reset()
	...
	if open and not open_prev and not close:
		anim.play("Open")
	...
	debug('close: ' + String(close))
	debug('close_prev: ' + String(close_prev))
